...
export class MessagingModule {
  static forRoot(options: MessagingModuleOptions = {}): DynamicModule {
    ...
    if (enablePublisher) providers.push(RollupPublisherService);
    if (enableConsumer) providers.push(RollupConsumerService);
    if (enableCron) providers.push(RollupCronService);

    const exports: Provider[] = [RabbitConnectionService];
    if (enablePublisher) exports.push(RollupPublisherService);
